...
- 9/3/2024 - We added some code to read the profiling function code and then store it in a column named 'source_code_compressed' in the final code profiling results output Delta table.  We compress the source code using 'base64' and 'zlib' Python libraries.  There is an example of how to use a Python UDF to decode the 'source_code_compressed' column into a 'source_code_decompressed' column with the original source code.  This example can be found in __unit_testing --> python --> code_profiler_test_py_files.py__.  This can then be used with any external LLM to get function level optimization recommendations.